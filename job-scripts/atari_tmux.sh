...
# Rename the first window and send the command
tmux rename-window -t atari_session:0 'Pong-relu'
tmux send-keys -t atari_session:0 'python3 discovery/experiments/FeatAct_atari/run_atari.py \
...
tmux new-window -t atari_session:1 -n 'Seaquest-relu'
...
tmux new-window -t atari_session:2 -n 'MsPacman-relu'
...
tmux new-window -t atari_session:3 -n 'Pong-fta'
...
tmux new-window -t atari_session:4 -n 'Seaquest-fta'
...
tmux new-window -t atari_session:5 -n 'MsPacman-fta'
tmux send-keys -t atari_session:5 'python3 discovery/experiments/FeatAct_atari/run_atari.py \
    --env_name "ALE/MsPacman-v5" \
    --activation "fta" \
    --feat_dim 10240 \
    --use_wandb' C-m
