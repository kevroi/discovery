...
for opt_id in range(len(opt_policies)):
    for g_id in range(len(states_rc)):
        for s_id in range(len(states_rc)):
            steps, ns = simulate_opt(opt_policies[opt_id], states_rc, s_id, g_id)
    ...
    for g_id, GOAL in enumerate(states_rc):
        ...
        while delta > tolerance:
            ...
            for s_id, s in enumerate(states_rc):
                ...
                v = V[r][c]
                va = 0
                p = 1.0 / (len(default_action_set) + num_options)
        ...
        if (g_id + 1) % 10 == 0:
            sys.stdout.write("\n")
        sys.stdout.flush()
    out = "Num Options: {} | Diffusion Time: {}".format(num_options, np.mean(means))
    print(out)
