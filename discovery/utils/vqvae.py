...
def max_one_hot(
    logits: torch.Tensor, straight_through_grads: bool = True, dim: int = -1
):
    """
...
class VectorQuantizerEMA(nn.Module):
    ...
    def forward(
        self, inputs, codebook_mask=None, masked_input=True, masked_output=True
    ):
        """
        ...
        oh_encodings = rearrange(
            flat_oh_encodings, "(b n) d -> b d n", b=inputs.shape[0]
        )
        ...
        if self.training:
            self._ema_cluster_size = self._ema_cluster_size * self._decay + (
                1 - self._decay
            ) * torch.sum(flat_oh_encodings.detach(), 0)
...
class VQVAEModel(nn.Module):
    def __init__(
        ...
        self.quantizer = VectorQuantizerEMA(
            codebook_size, embedding_dim, commitment_cost=0.25, decay=0.99
        )
        ...
        if n_latents is not None:
            if len(self.encoder_out_shape) == 1:
                ...
                raise NotImplementedError(
                    "`n_latents` param not supported for >1-D encoder outputs!"
                )
        elif len(self.encoder_out_shape) == 1:
            raise ValueError(
                "VQVAEs with dense encoders must have a value for `n_latents`!"
            )
    ...
    def create_sparsity_mask(self, sparsity=0.0, sparsity_type="random"):
        if self.encoder_type == "soft_vqvae" and (
            ...
            if sparsity_type == "random":
                ...
                assert (
                    self.n_embeddings == self.embedding_dim
    ...
    def get_encoder(self):
        ...
        encoder = nn.Sequential(self.encoder, new_quantizer)
    ...
    def forward(self, x):
        ...
        if self.sparsity_enabled and self.sparsity_mask is not None:
            ...
            quantized = self.quantizer(
                encoder_out, self.sparsity_mask, masked_input=False
            )[1]
        else:
            quantizer_loss, quantized, perplexity, oh_encodings = self.quantizer(
                encoder_out
            ...
            x_hat = self.decoder(quantized)
        out_encodings = (
            quantized.reshape(x.shape[0], self.n_embeddings, -1)
            if self.quantized_enc
            else oh_encodings
        )
    ...
    def decode(self, encodings):
        if self.quantized_enc:
            ...
            if self.sparsity_enabled and self.sparsity_mask is not None:
                ...
                quantized = self.quantizer(
                    quantized, self.sparsity_mask, masked_output=False
                )[1]
        else:
