    ...
    def start(self):
        ...
        if self.start_state == (-1, -1):
            valid_state_idx = [
                ...
                if state not in self.obstacle_vector
            ]
    ...
    def step(self, action):
        ...
        if action == TERMINATE_ACTION:
            ...
            if s in self.obstacle_vector or ns in self.obstacle_vector:
                ...
                reward = (
                    self.reward_vector[ns] - self.reward_vector[s]
                )  # - 0.001 # small step penalty
        ...
        if (
            self.goal_state != (-1, -1)
            and self.states_rc.index(self.goal_state) == self.current_state[0]
        ):
...
class GridEnvironment(BaseEnvironment):
    def __init__(self):
        ...
        max_row, max_col, start_state, goal_state, obstacles, lava, _, _ = parse_env(
            grid_env
        )
        ...
        BaseEnvironment.__init__(
            self, max_row, max_col, start_state, goal_state, obstacles, lava
        )
...
class phi1Environment(BaseEnvironment):
    def __init__(self, name):
        room_env = to_env_list(f"environments/env_layouts/{name}.txt")
        max_row, max_col, start_state, goal_state, obstacles, lava, phi_1, phi_2 = (
            parse_env(room_env)
        )
        self.max_row = max_row
        self.max_col = max_col
    ...
    def step(self, action):
        ...
        if action == TERMINATE_ACTION:
            ...
            if ns in self.phi_1:
                ns = self.phi_1[
                    1
                ]  # Assuming all states in phi_1 are treated as the same state
...
def plot_gridworld(file_path, hallways=None):
    ...
    plt.imshow(
        [[colour_from_letter(cell) for cell in row] for row in grid],
        cmap="gray",
        interpolation="none",
        aspect="equal",
    )
...
if __name__ == "__main__":
    file_path = (
        "environments/env_layouts/2room.txt"  # Replace with the path to your text file
    )
