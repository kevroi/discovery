...
def plot_wandb_data(project_name="SubgoalFeats_CosSim",
    ...
    df = pd.read_csv(f"experiments/FeatAct_minigrid/wandb_export_data/{project_name}_{env_name}_{learner}_{activation}_{feat_dim}_data.csv")
    mean_rewards = df.iloc[:, 1:].mean(axis=1)  # Ignore the first column (index) and calculate mean
    std_rewards = df.iloc[:, 1:].std(axis=1)    # Ignore the first column (index) and calculate standard deviation
