...
results = np.load(f'experiments/feature_attainment/data_files/{env_name}_average_return.npy')

plt.show()
x_legend = range(len(results[:]))
graph_agent_0, = plt.plot(x_legend, results, label="primitive actions")
